...
def buy():
    ...
    if request.method == "GET":
        ...
        # update history
        db.execute("INSERT INTO histories (symbol, shares, price, user_id, transacted) \
                    VALUES(:symbol, :shares, :price, :id, :transacted)", \
                    symbol=stock["symbol"], shares=shares, \
                    price=usd(stock["price"]), id=session["user_id"], transacted = "buy")
        ...
        if not user_shares:
            db.execute("INSERT INTO portfolio (name, shares, price, total, symbol, user_id) \
...
def history():
    ...
    histories = db.execute("SELECT * from histories WHERE user_id=:id", id=session["user_id"])
...
def sell():
    ...
    if request.method == "GET":
        ...
        db.execute("INSERT INTO histories (symbol, shares, price, user_id, transacted) \
                    VALUES(:symbol, :shares, :price, :id, :transacted)", \
                    symbol=stock["symbol"], shares=-shares, \
                    price=usd(stock["price"]), id=session["user_id"], transacted = "sell")
